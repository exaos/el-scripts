...
#Time-stamp: <2012-02-09 18:43:08 Thursday by roowe>
...
#        urls.append((name, url))
    for id, name in re.findall(r'<input name="bt_taskname".+?id="bt_taskname(\w+?)".+?value="(.+?)"', html):
        result = re.search(r'<input id="bt_list%s".+?value="(http.*?)"'%id, html)
        if result:
            name = re.sub("&(.*?);", handle_entitydef, name)
            url = result.group(1)
            url = re.sub("&(.*?);", handle_entitydef, url)
...
def collect_urls_with_cloud(html):
    urls = []
    for id, name in re.findall(r'<input id=[\'"]durl(\w+?)[\'"].+title=[\'"](.+?)[\'"].+', html):
        result = re.search(r'<input id=[\'"]cloud_dl_url%s[\'"].+value=[\'"](http.*?)[\'"]' % id, html)
        if result:
            name = re.sub("&(.*?);", handle_entitydef, name)
            url = result.group(1)
            url = re.sub("&(.*?);", handle_entitydef, url)
...
def thuner_xl_with_wget(urls, output_dir, cookies_file, limit_rate=None, quiet=False):
    ...
    for name, url in urls:
        ...
        if not download_list[name]:
            continue
        cmd = ["wget", "--load-cookies", cookies_file, "-c", "-t", "5",  "-U", "Mozilla", "-O", os.path.join(output_dir, name), url]
        if quiet:
            cmd.insert(1, "-q")
        if limit_rate:
...
def user_task_page(cookies_file, page_file):
    ...
    cmd = ["wget", "--load-cookies", cookies_file, "-t", "5", "-U", "Mozilla", "-O", page_file, page_url]
    ret = subprocess.call(cmd)
    if ret != 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', nargs='?', default="/tmp/user_task.html", help="load page file")
    parser.add_argument('-c', nargs='?', default="/tmp/cookies.txt", help="load cookie file")
    ...
    parser.add_argument('-u', action="store_true", default=False, help="get user task page")
    parser.add_argument('-y', action="store_true", default=False, help="cloud trans")
